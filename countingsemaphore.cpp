...
#define NO_OF_QUEUE      2	/* # process in queue */
#define NO_OF_SLOTS      1    	/*Capacity  */
...
void *Supervisor(void *arg){
	while( c < -5) {			/* Resources limit */
	int student = (uintptr_t)arg;
...
void *Student(void *arg){
	...
	while (studentRemoved < 2){
		...
		student = Q[front];
		printf("%4d. Process is done. He was removed from the queue \n", student+1);
		studentRemoved++;
...
int main(int argc,char *argv[]){
	...
	for(i=0; i < 2;i++){
		pthread_create(&supervisorid, NULL, Supervisor , &i);
	} 
